    ...
    if (selectedNodes.length > 0) {
      ...
      for (let i = 0; i < selectedNodes.length; i++) {
        if (this.body.nodes[selectedNodes[i]].isCluster === true) {
          let cluster = Object.assign({}, this.body.nodes[selectedNodes[i]])

          for (const key in cluster.containedNodes) {
      ...
      selectedNodes = selectedNodes.concat(internalNodes)
      ...
      for (let i = 0; i < selectedEdges.length; i++) {
        if (selectedEdges[i].includes("clusterEdge")){
            let clusterInEdge = Object.assign({}, this.body.edges[selectedEdges[i]])
            
            for (const xid in clusterInEdge.clusteringEdgeReplacingIds) {
              if (clusterInEdge.clusteringEdgeReplacingIds.hasOwnProperty(xid)) {
                const replacedId = clusterInEdge.clusteringEdgeReplacingIds[xid];
                ...
                let edgeRepl = this.body.edges[replacedId]
                let dataEdgeRepl = {id: edgeRepl.id, from: edgeRepl.fromId, to: edgeRepl.toId}
            ...
            let edgeRepl = this.body.edges[selectedEdges[i]]
            let dataEdgeRepl = {id: edgeRepl.id, from: edgeRepl.fromId, to: edgeRepl.toId}
            this.body.signalCallback("deleteEdges", dataEdgeRepl)
    ...
    if (new Date().valueOf() - this.touchTime > 100) {
      ...
      if (node !== undefined) {
        if (node.isCluster === true) {
          ...
          let internal_node = {}
          for (const key in node.containedNodes) {
